...
export class NetNode {
    ...
    protected _receiveTime: number = 600000;                                // 多久没收到数据断开
    protected _reconnetTimeOut: number = 600000;                            // 重连间隔
    ...
    protected onConnected(event: any) {
        ...
        console.log(`网络已连接当前状态为【${NetNodeStateStrs[this._state]}】`);
    }

    /** 连接验证成功，进入工作状态 */
    protected onChecked() {
        console.log("连接验证成功，进入工作状态");
        ...
        if (requests.length > 0) {
            console.log(`请求【${this._requests.length}】个待发送的信息`);
    ...
    protected onMessage(msg: any): void {
        // console.log(`接受消息状态为【${NetNodeStateStrs[this._state]}】`);
        ...
        if (this._requests.length > 0) {
            ...
            if (this._requests.length == 0) {
                ...
                console.log(`请求队列中还有【${this._requests.length}】个请求在等待`);
            }
        }
    ...
    send(buf: NetData, force: boolean = false): number {
        if (this._state == NetNodeState.Working || force) {
            ...
            console.log(`当前状态为【${NetNodeStateStrs[this._state]}】,繁忙并缓冲发送数据`);
            return 0;
        }
        else {
